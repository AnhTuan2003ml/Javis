    ...
    def _init_ai_models(self):
        if self.ai_provider == 'groq':
            try:
                ...
                from core.ai.groq_config import GROQ_API_KEY
    ...
    def execute(self, query):
        try:
            ...
            if 'set reminder' in query.lower() or 'remind me' in query.lower() or 'task reminder' in query.lower():
                try:
                    from core.ai.new_features import task_reminder
            ...
            if 'show calendar' in query.lower() or 'check calendar' in query.lower() or 'add event' in query.lower():
                try:
                    from core.voice.voice_advanced_ai import get_voice_advanced_response
            ...
            if query_lower in self.functions:
                ...
                if not func_name:
                    ...
                    try:
                        from core.ai.new_features import get_new_feature_response
...
RESPONSE:'''
    ...
    def _switch_to_hindi(self):
        if self.multilingual and self.multilingual.set_language('hindi'):
            ...
            from core.voice.multilingual_support import reload_multilingual
            reload_multilingual()
            self.multilingual = multilingual
    ...
    def _enable_face_auth(self):
        try:
            from core.config.face_auth_config import set_face_auth_status
    ...
    def _get_face_auth_status(self):
        try:
            from core.config.face_auth_config import get_face_auth_status
    ...
    def _create_video(self):
        ...
        try:
            ...
            from core.media.simple_video_gen import create_simple_video
...
# _ai_presentation("AI", create_ppt=True)        # Creates actual PowerPoint file
    ...
    def _ai_dj_mode(self):
        try:
            from core.ai.ai_dj_enhanced import ai_dj_mode_enhanced
    ...
    def _storytelling(self):
        try:
            from core.voice.voice_advanced_ai import VoiceAdvancedAI
            ai = VoiceAdvancedAI()
...
Max 2 lines each.'''
    ...
    def _save_location(self, location=""):
        try:
            ...
            with open('data/state/saved_locations.txt', 'a', encoding='utf-8') as f:
                f.write(f"{location}\n")
    ...
    def _dictate_to_file(self, query=""):
        ...
        try:
            ...
            filename = "data/state/dictation.txt"
    ...
    def _extract_command_argument(self, query, keywords):
        """Extract a free-form argument from the current command without opening a dialog."""
        text = query.strip()
        for keyword in keywords:
            if keyword in text:
                value = text.split(keyword, 1)[1].strip()
                if ' in ' in value:
                    value = value.split(' in ', 1)[0].strip()
                return value.strip('"\' ')
        return ""
    ...
    def _find_file(self):
        ...
        try:
            ...
            # Get search pattern
            pattern = self._extract_command_argument(query, [
                'find file', 'find files', 'search file', 'search files'
            ])
            if not pattern:
                return "Please include a filename or pattern, for example: find file report in downloads"
    ...
    def _get_file_info(self):
        ...
        try:
            ...
            file_name = self._extract_command_argument(query, [
                'file info', 'get file info', 'file information', 'info file'
            ])
            if not file_name:
                return "Please include a filename, for example: file info report.txt in documents"
    ...
    def _backup_folder(self):
        ...
        try:
            ...
            folder_path = self._extract_command_argument(query, [
                'backup folder', 'backup directory', 'back up folder', 'back up directory'
            ])
            if not folder_path:
                return "Please include a folder path, for example: backup folder D:\\Projects\\Demo"
    ...
    def _search_content(self):
        ...
        try:
            ...
            search_term = self._extract_command_argument(query, [
                'search content', 'search inside files', 'find content', 'content search'
            ])
            if not search_term:
                return "Please include text to search for, for example: search content invoice in documents"
    ...
    def _suggest_folder(self):
        ...
        try:
            query = self._current_query.lower()
            file_path = self._extract_command_argument(query, [
                'smart folder', 'suggest folder', 'analyze file', 'file to analyze'
            ])
            if not file_path:
                return "Please include a file path, for example: smart folder D:\\Documents\\report.pdf"
        ...
        try:
            ...
            if duration > 0:
                def countdown():
                    ...
                    print("\n✅ Time's up!")
                    try:
                        from core.commands.command import speak
                        speak("Time's up!")
                    except:
                        pass
    ...
    def _create_image(self):
        ...
        try:
            ...
            from core.media.simple_image_gen import create_simple_image
    ...
    def _set_alarm(self):
        ...
        try:
            ...
            # Save to file
            with open('data/state/alarm.json', 'w') as f:
    ...
    def _cancel_alarm(self):
        ...
        try:
            if self.active_alarm:
                ...
                # Remove file
                import os
                if os.path.exists('data/state/alarm.json'):
                    os.remove('data/state/alarm.json')
