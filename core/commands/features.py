...
from core.config.config import ASSISTANT_NAME
...
from core.commands.helper import extract_yt_term, remove_words
...
def hotword():
    if not PVPORCUPINE_AVAILABLE or not PYAUDIO_AVAILABLE:
        ...
        try:
            from core.voice.simple_hotword import simple_hotword_detection
...
def chatBot(query):
    ...
    try:
        ...
        if os.path.exists("core\\cookies.json"):
            chatbot = hugchat.ChatBot(cookie_path="core\\cookies.json")
...
def testSMS():
    ...
    from core.commands.command import takecommand
